    ...
    if (data.claude_state !== null) {
      if (/#claude\s\S.*/.test(l) && !claudeAdded) {
        ...
        if (response != null && response !== "") {
          ...
          html +=
            '<div class="claude-container" hidden>\n' +
            '<div class="blockquote-divider"></div>\n' +
...
function wrapTags(level: number, state: TextState): string {
  ...
  const closing = ["</del>", "</b>", "</i>", "</span>", "</span>"];
...
function parseSpoilers(
  ...
  const _fn = (frag: string) => parseBolds(frag, state, fn);
  let html = "";
  while (true) {
    const i = frag.indexOf("**");
    if (i !== -1) {
...
function parseBolds(
  ...
  const _fn = (frag: string) => parseItalics(frag, state, fn);
  let html = "";
  while (true) {
    const i = frag.indexOf("@@");
    if (i !== -1) {
...
function parseReds(
  ...
  const _fn = (frag: string) => parseBlues(frag, state, fn);
...
function parseFragment(frag: string, data: PostData): string {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    switch (word[0]) {
      case "#": // Hash commands
        if (data.state.quote || data.state.reversequote) {
          break;
        }
        m = word.match(
    ...
    if (!matched) {
      ...
      for (let i = 0; i < chars.length; i++) {
        const ch = chars[i];
        if (ch === "回" && i + 1 < chars.length && chars[i + 1] === "レ") {
