...
function quotePost(e: MouseEvent) {
  ...
  const isInside = (prop: string): boolean => {
    ...
    if (!el) {
      // No idea why, but el sometimes is null
      return false;
    }
...
function addFlvFrame(e: MouseEvent) {
  const videoElement: HTMLVideoElement = <HTMLVideoElement>(
    document.getElementById("flv-player")
  );
...
function addMeguFrame(e: MouseEvent) {
  const videoElement: HTMLVideoElement = <HTMLVideoElement>(
    document.getElementById("megu-tv-video")
  );
  addStreamFrame(videoElement);
}
...
function addStreamFrame(videoElement: HTMLVideoElement) {
  ...
  canvas.toBlob((blob) => {
    if (blob) {
      ...
      const jpgFile = new File([blob], `frame-${ts}.jpg`, {
        type: "image/jpeg",
      });
      // On board pages do nothing
      if (!state.page.thread) {
...
export default () => {
  ...
  postSM.act(postState.none, postEvent.sync, () => postState.ready);
  ...
  postSM.act(postState.halted, postEvent.reclaim, () => postState.alloc);

  // Regained connectivity too late and post can no longer be reclaimed
  postSM.act(postState.halted, postEvent.abandon, () => postState.ready);

  // Regained connectivity, when no post open
  postSM.act(postState.locked, postEvent.sync, () => postState.ready);
  ...
  postSM.wildAct(postEvent.reset, () => postState.ready);

  // Transition a draft post into allocated state. All the logic for this is
  // model- and view-side.
  postSM.act(postState.allocating, postEvent.alloc, () => postState.alloc);
  ...
  on(
    ...
    "click",
    (e) => {
      options.meguTV = false;
    },
    {
      selector: "#megu-close-button",
    },
  );
