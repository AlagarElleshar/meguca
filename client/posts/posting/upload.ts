...
export default class UploadForm extends View<Post> {
  ...
  constructor(model: Post, el: HTMLElement, attachTiktokCallback: () => void) {
    super({ el, model });
    el.hidden = false;
    this.spoiler = el.querySelector(
      `span[data-id="spoiler"]`,
    ) as HTMLInputElement;
    this.hiddenInput = el.querySelector(
      "input[name=image]",
    ) as HTMLInputElement;
    this.mask = el.querySelector(`span[data-id="mask"]`) as HTMLInputElement;
    ...
    this.inputElement("mask").checked = localStorage.getItem("mask") === "true";

    this.mask.addEventListener(
      "click",
      () => {
        localStorage.setItem("mask", String(this.inputElement("mask").checked));
      },
      { passive: true },
    );
            ...
            .then((stream) => {
              ...
              this.recorder.onstop = () => {
                trigger("getPostModel").uploadFile(
                  new File(this.audioChunks, "recording.ogg"),
                );
...
export function initUpload() {
  ...
  if (!page.thread) {
    ...
    const mask = form.querySelector("input[name=mask]") as HTMLInputElement;
