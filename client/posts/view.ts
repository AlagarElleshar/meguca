...
export default class PostView extends ImageHandler {
  ...
  private buffer(): Element {
    const {
      state: { spoiler, quote, bold, italic, red, blue },
    } = this.model;
  ...
  public renderBacklinks() {
    ...
    let el = firstChild(this.el, (ch) => ch.classList.contains("backlinks"));
    if (!el) {
  ...
  public renderTime(recursive = false) {
    ...
    const timeUntilInterval =
      intervalPeriod - ((timeDelta * 1000) % intervalPeriod);
    setTimeout(() => {
      this.renderTimeOnce();
      ...
      const timeUntilNextInterval =
        nextInterval - ((timeDelta * 1000) % nextInterval);
      if (nextInterval) {
        setTimeout(() => {
          clearInterval(id);
  ...
  private readableTime(): string {
    ...
    return (
      `${pad(d.getDate())} ${lang.time.calendar[d.getMonth()]} ` +
  ...
  public renderName() {
    ...
    if (auth) {
      ...
      html += `<span>## ${lang.posts[modLevelStrings[auth]] || "??"}</span>`;
    }
    if (mine.has(id)) {
      ...
      switch (type) {
        case ModerationAction.banPost:
          s = this.format(
            "banned",
            by,
            secondsToTime(length).toUpperCase(),
            data,
          );
          break;
  ...
  private format(formatKey: string, ...args: string[]): string {
    let i = 0;
    return lang.format[formatKey].replace(/%s/g, (_) => args[i++]);
  }
